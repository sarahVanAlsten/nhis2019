...
eligible <- eligible %>%
  mutate(Asthma = ifelse(ASTHMASTIL ==2, 1,
                         ifelse(ASTHMASTIL %in% c(0,7,8,9), NA, 0))) #Recode asthma Ever to Binary Variable

eligible <- eligible %>%
  mutate(DEAD = ifelse(MORTSTAT ==1, 1,
                         ifelse(MORTSTAT ==2, 0,NA))) #Recode asthma Ever to Binary Variable
...
names(eligible)
eligible$numMedBx <- rowSums(eligible[,404:408])
...
names(eligible)
#How many cost related barriers to care they had
eligible$numCostBarrier <- rowSums(eligible[,c(393,399:403)])
eligible$anyCostBarrier <- ifelse(eligible$numCostBarrier > 0, 1, 0)

#how many cost related barrieres minus Mental Health
eligible$numCostBarrierNoMH <- rowSums(eligible[,c(393,399:402)])
...
inconFU <- eligible[eligible$fuTime < 0 & !is.na(eligible$fuTime), c("enrollDate", "deathDate",
                                                          "DiabetesRec", "CancerEvBin", "HyperTen", "CHD", "HeartDz")]
...
eligible <- eligible[eligible$fuTime > 0 | is.na(eligible$fuTime),]
rm(subData)
rm(inconFU)
...
f1 <- coxph(Surv(fuTime, DEAD)~anyMedBx +AGE+SEX+BMI, data = diabDat, weights = MORTWTSA)
summary(f1)
f1 <- coxph(Surv(fuTime, DEAD)~anyCostBarrierNoMH +AGE+SEX+BMI, data = diabDat, weights = MORTWTSA)
summary(f1)
